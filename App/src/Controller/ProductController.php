...
final class ProductController extends AbstractController
{
    ...
    public function index(ProductRepository $productRepository, Request $request, PaginatorInterface $paginator): Response
    {
        ...
        $query = $productRepository->findAllProductsQuery($search, $sortField, $sortOrder);

        // dd($query);

        // Fetch current page number (default to 1 if not provided)
        $page = $request->query->getInt('page', 1);

        // Define the limit of results per page
        $limit = 4;
